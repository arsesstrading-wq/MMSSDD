...
const markdownToHtml = (text: string): string => {
  ...
  lines.forEach(line => {
      ...
      const isListItem = processedLine.trim().startsWith('* ') || processedLine.trim().startsWith('- ');
      ...
      if (isListItem) {
          ...
          // Use a paragraph for non-list lines for better structure and spacing
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50" onClick={onClose}>
      ...
      >
        ...
        <div className="overflow-y-auto ltr:pr-2 rtl:pl-2">
            <div className="text-gray-700 dark:text-gray-300 prose prose-sm dark:prose-invert max-w-none [&_ul]:my-2 [&_p]:my-2" dangerouslySetInnerHTML={{ __html: formattedMessage }}></div>
        </div>
      </div>
    </div>
  );
