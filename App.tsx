...
import type { View, User, LogEntry, ModalContent, SummaryTimeFrame, Profile, ColorTheme, Language, DisplayTheme, Achievement, Reminder } from './types';
import { DEFAULT_PROFILE } from './constants';
import { 
    GlucoseMeterIcon, UserIcon, PlusIcon,
    HistoryIcon, BloodDropIcon, MealIcon, ActivityIcon, SleepIcon,
    SyringeIcon, MoodIcon, GridIcon, MedicationIcon, BodyIcon
...
const themeColorPalettes = {
  indigo: { '50': '238 242 255', '100': '224 231 255', '200': '199 210 254', '300': '165 180 252', '400': '129 140 248', '500': '99 102 241', '600': '79 70 229', '700': '67 56 202', '800': '55 48 163', '900': '49 46 129', '600-val': '#4f46e5', '400-val': '#818cf8', 'gradient-start': '#6366f1', 'gradient-end': '#4338ca' },
  pink: { '50': '253 242 248', '100': '252 231 243', '200': '251 207 232', '300': '249 168 212', '400': '244 114 182', '500': '236 72 153', '600': '219 39 119', '700': '190 24 93', '800': '157 23 77', '900': '131 24 67', '600-val': '#db2777', '400-val': '#f9a8d4', 'gradient-start': '#ec4899', 'gradient-end': '#be185d' },
  blue: { '50': '239 246 255', '100': '219 234 254', '200': '191 219 254', '300': '147 197 253', '400': '96 165 250', '500': '59 130 246', '600': '37 99 235', '700': '29 78 216', '800': '30 64 175', '900': '30 58 138', '600-val': '#2563eb', '400-val': '#60a5fa', 'gradient-start': '#3b82f6', 'gradient-end': '#1d4ed8' },
  green: { '50': '240 253 244', '100': '220 252 231', '200': '187 247 208', '300': '134 239 172', '400': '74 222 128', '500': '34 197 94', '600': '22 163 74', '700': '21 128 61', '800': '22 101 52', '900': '20 83 45', '600-val': '#16a34a', '400-val': '#4ade80', 'gradient-start': '#22c55e', 'gradient-end': '#15803d' },
  orange: { '50': '255 247 237', '100': '255 237 213', '200': '254 215 170', '300': '253 186 116', '400': '251 146 60', '500': '249 115 22', '600': '234 88 12', '700': '194 65 12', '800': '154 52 18', '900': '124 45 18', '600-val': '#ea580c', '400-val': '#fb923c', 'gradient-start': '#f97316', 'gradient-end': '#c2410c' },
  teal: { '50': '240 253 250', '100': '204 251 241', '200': '153 246 228', '300': '94 234 212', '400': '45 212 191', '500': '20 184 166', '600': '13 148 136', '700': '15 118 110', '800': '17 94 89', '900': '19 78 74', '600-val': '#0d9488', '400-val': '#2dd4bf', 'gradient-start': '#14b8a6', 'gradient-end': '#0f766e' },
  red: { '50': '254 242 242', '100': '254 226 226', '200': '254 202 202', '300': '252 165 165', '400': '248 113 113', '500': '239 68 68', '600': '220 38 38', '700': '185 28 28', '800': '153 27 27', '900': '127 29 29', '600-val': '#dc2626', '400-val': '#f87171', 'gradient-start': '#ef4444', 'gradient-end': '#b91c1c' },
  amber: { '50': '255 251 235', '100': '254 243 199', '200': '253 230 138', '300': '252 211 77', '400': '251 191 36', '500': '245 158 11', '600': '217 119 6', '700': '180 83 6', '800': '146 64 14', '900': '120 53 15', '600-val': '#d97706', '400-val': '#fbbd23', 'gradient-start': '#f59e0b', 'gradient-end': '#b45309' },
  rose: { '50': '255 241 242', '100': '255 228 230', '200': '254 205 211', '300': '253 164 175', '400': '251 113 133', '500': '244 63 94', '600': '225 29 72', '700': '190 18 60', '800': '159 18 57', '900': '136 19 55', '600-val': '#e11d48', '400-val': '#fb7185', 'gradient-start': '#f43f5e', 'gradient-end': '#be123c' },
  slate: { '50': '248 250 252', '100': '241 245 249', '200': '226 232 240', '300': '203 213 225', '400': '148 163 184', '500': '100 116 139', '600': '71 85 105', '700': '51 65 85', '800': '30 41 59', '900': '15 23 42', '600-val': '#475569', '400-val': '#94a3b8', 'gradient-start': '#64748b', 'gradient-end': '#334155' },
};
...
const ACHIEVEMENTS: Record<string, Achievement> = {
  FIRST_LOG: {
    id: 'FIRST_LOG',
    name: 'First Step!',
    description: 'You made your first log.',
  }
};
...
const BottomNavBar = React.memo<{
    ...
    return (
        <div style={{ paddingBottom: 'var(--safe-area-inset-bottom)'}} className="fixed bottom-0 left-0 right-0 h-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-t border-gray-200 dark:border-gray-700 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.2)] flex justify-around items-center z-20">
            {navItems.map((item) => {
                 const isActive = activeView === item.view;
                 if (item.view === 'add-placeholder') {
                     return (
                        <div key="add-button-wrapper" className="relative w-1/5 h-full flex justify-center">
                            <button
                                onClick={onAddClick}
                                style={{ background: 'linear-gradient(to top right, var(--gradient-start), var(--gradient-end))' }}
                                className="absolute -top-8 text-white rounded-full w-16 h-16 flex items-center justify-center shadow-lg hover:opacity-90 active:scale-95 transition-all duration-200"
                                aria-label={t('newLog')}
                            >
                ...
                return (
                    ...
                    >
                        <item.icon isActive={isActive} className="h-6 w-6" />
...
const LogMenu = React.memo<{ onClose: () => void; onSelect: (view: View) => void; t: (key: keyof typeof strings.fa) => string; }>(({ onClose, onSelect, t }) => {
...
const App: React.FC = () => {
  ...
  const [lastSyncTime, setLastSyncTime] = useState<string | null>(() => localStorage.getItem('diab-lastSyncTime'));
  ...
  useEffect(() => {
    ...
    // Delay init to allow scripts to load
    setTimeout(() => googleDriveService.initClient(onAuthChange), 1000);
  }, []);
  ...
  useEffect(() => {
    ...
    if (action) {
      const validViews: View[] = ['logBloodSugar', 'logMeal'];
      if (validViews.includes(action as View)) {
    ...
    try {
        ...
        if (storedUsers) {
            parsedUsers = JSON.parse(storedUsers).map((u: any) => ({
                ...
                avatar: u.avatar || { hat: false },
            }));
        }

        if (parsedUsers.length === 0) {
            // Don't create a user yet, let the WelcomeScreen handle it.
        } else {
  ...
  useEffect(() => {
    ...
    try {
        ...
        if (lastSyncTime) localStorage.setItem('diab-lastSyncTime', lastSyncTime);

    } catch (error) {
        console.error("Failed to save data to localStorage", error);
    }
  }, [users, selectedUserId, displayTheme, colorTheme, language, summaryTimeFrame, loading, lastSyncTime]);
  ...
  useEffect(() => {
    ...
    Object.entries(colors).forEach(([key, value]) => {
      const propKey = key.includes('-val') 
        ? `--color-primary-${key}` 
  ...
  // Notification scheduler for reminders
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      currentUser.reminders.forEach(reminder => {
        if (reminder.enabled && reminder.time === currentTime) {
          const lastTime = lastNotified.get(reminder.id);
          // Only notify once per minute for a given reminder
          if (!lastTime || (now.getTime() - lastTime > 60000)) {
            const title = t('appName');
            const body = reminder.type === 'check_bg' 
              ? t('reminderBody_check_bg') 
              : t('reminderBody_take_meds');
            ...
            lastNotified.set(reminder.id, now.getTime());
          }
        }
      });
  ...
  const handleSetView = useCallback((view: View, payload: any = null) => {
    setView(view);
  ...
  const checkAndAwardAchievements = useCallback((user: User): User => {
    const updatedUser = { ...user };
    const LEVEL_THRESHOLD = 100;

    // Award for first log
    if (user.logs.length >= 1 && !user.achievements.includes('FIRST_LOG')) {
        updatedUser.achievements = [...user.achievements, 'FIRST_LOG'];
        updatedUser.xp += 50; // Bonus XP for first achievement
        // Here you could set a modal to announce the achievement
    }
    
    // Check for level ups to award items
    const level = Math.floor(updatedUser.xp / LEVEL_THRESHOLD) + 1;
    if (level >= 2 && !updatedUser.avatar.hat) {
        updatedUser.avatar = { ...updatedUser.avatar, hat: true };
    }

    return updatedUser;
  }, []);

  const applyRestoredData = useCallback((data: string) => {
      try {
  ...
  const handleAddUser = (name: string, profileDetails: Partial<Profile>) => {
    const newUser: User = { 
        id: `user_${Date.now()}`, 
        name, 
        profile: { ...DEFAULT_PROFILE, ...profileDetails }, 
        logs: [], 
        reminders: [], 
        goals: [],
        xp: 0,
  ...
  const handleSaveLog = (logData: Omit<LogEntry, 'id' | 'timestamp'>) => {
    ...
    const getBaseDateForTimestamp = (): Date => {
      if (editingLog) return new Date(editingLog.timestamp);
      return new Date();
    };

    const baseDate = getBaseDateForTimestamp();
    const [hours, minutes] = finalLogData.time.split(':').map(Number);
    const newTimestamp = new Date(baseDate);
    newTimestamp.setHours(hours, minutes, 0, 0);

    let userAfterLogUpdate: User;

    if (editingLog) {
      const updatedLogs = currentUser.logs.map(log =>
        log.id === editingLog.id
          ? { ...log, ...finalLogData, timestamp: newTimestamp }
          : log
      );
      userAfterLogUpdate = { ...currentUser, logs: updatedLogs };
    } else {
      ...
      userAfterLogUpdate = { 
        ...currentUser, 
        logs: [...currentUser.logs, newLog],
        xp: (currentUser.xp || 0) + XP_PER_LOG,
      };
    }

    const finalUser = checkAndAwardAchievements(userAfterLogUpdate);
  ...
  }, [currentUser, isEstimating, isAiEnabled, t]);

  const handleStartComparison = useCallback((predictedValue: number) => {
  ...
  const confirmRestore = () => {
    ...
    const success = applyRestoredData(restoreData);
    if (success) {
  ...
  const handleGoogleSync = async (): Promise<boolean> => {
    const backupData = getBackupData();
    const success = await googleDriveService.uploadBackup(backupData);
    if(success) {
        const newSyncTime = new Date().toISOString();
        setLastSyncTime(newSyncTime);
        localStorage.setItem('diab-lastSyncTime', newSyncTime);
    }
    return success;
  };

  const handleGoogleRestore = async (): Promise<boolean> => {
      const backupData = await googleDriveService.restoreBackup();
      if(backupData) {
          const success = applyRestoredData(backupData);
          return success;
      }
      return false;
  };
  ...
  const openLogMenu = useCallback(() => setIsLogMenuOpen(true), []);
  const closeLogMenu = useCallback(() => setIsLogMenuOpen(false), []);
  const selectLogMenu = useCallback((logView: View) => {
    closeLogMenu();
    setTimeout(() => handleSetView(logView), 100);
  }, [closeLogMenu, handleSetView]);
  ...
  const renderContent = () => {
    ...
    if (users.length === 0 || !currentUser) {
        return <WelcomeScreen onUserAdd={handleAddUser} t={t} language={language} />;
    }
    ...
    switch (view) {
      ...
      case 'reminders':
        return <RemindersScreen currentUser={currentUser} onUpdateUser={handleUpdateUser} setView={handleSetView} t={t} language={language} />;
      case 'dashboard':
      default:
        return <DashboardTimeline 
            ...
            canEstimate={isAiEnabled && isOnline}
            t={t}
            dynamicT={dynamicT}
            language={language}
          />;
    ...
    <>
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 font-sans flex">
        ...
        <div className="w-full h-screen bg-white dark:bg-gray-800 flex flex-col">
          {renderContent()}
          {showBottomNav && (
            <BottomNavBar activeView={view} setView={handleSetView} onAddClick={openLogMenu} t={t} />
          )}
          {isLogMenuOpen && (
            <LogMenu onClose={closeLogMenu} onSelect={selectLogMenu} t={t} />
          )}
        </div>
      </div>
      ...
      {deletingLogId && (
        <ConfirmationModal message={t('deleteLogConfirm')} onConfirm={handleDeleteLogConfirm} onCancel={() => setDeletingLogId(null)} />
      )}
