    ...
    const recentLogs = currentUser.logs
      ...
      .map(log => {
        ...
        if (log.type === 'meal') {
          ...
          return `${mealName}${description} با ${log.carbs} گرم کربوهیدرات در ${timestamp}.${insulinText}${fattyText}`;
        }
...
export const analyzeDataWithGemini = async (currentUser: User): Promise<string> => {
  ...
  const prompt = `
    ...
    **وظیفه شما:**
    ...
    - **در صورت افت قند خون:**
        - مواد غذایی مشخصی برای رفع افت پیشنهاد دهید. (مثال: "برای رفع افت قند، ۱۵ گرم کربوهیدرات سریع‌الجذب مثل نصف لیوان آبمیوه یا ۳ حبه قند مصرف کن.")
